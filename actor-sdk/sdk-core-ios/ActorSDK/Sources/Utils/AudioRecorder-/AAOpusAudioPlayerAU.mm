...
//#define kInputBus 1
//
//static const int AAOpusAudioPlayerBufferCount = 3;
...
//
//static AA_SYNCHRONIZED_DEFINE(filledBuffersLock) = PTHREAD_MUTEX_INITIALIZER;
//
...
//
//    AAAudioBuffer *_filledAudioBuffers[AAOpusAudioPlayerBufferCount];
...
//- (void)cleanup
//{
//    AA_SYNCHRONIZED_BEGIN(filledBuffersLock);
//
//    activeAudioPlayers.erase(_playerId);
//
//    for (int i = 0; i < AAOpusAudioPlayerBufferCount; i++)
//    {
...
//
//    AA_SYNCHRONIZED_END(filledBuffersLock);
//
...
//                        freedAudioBuffers = (AAAudioBuffer **)malloc(sizeof(AAAudioBuffer *) * AAOpusAudioPlayerBufferCount);
//                    freedAudioBuffers[freedAudioBufferCount] = self->_filledAudioBuffers[0];
//                    freedAudioBufferCount++;
//
//                    for (int i = 0; i < AAOpusAudioPlayerBufferCount - 1; i++)
//                    {
//                        self->_filledAudioBuffers[i] = self->_filledAudioBuffers[i + 1];
//                    }
//                    self->_filledAudioBuffers[AAOpusAudioPlayerBufferCount - 1] = NULL;
//
...
//            {
//                NSLog(@"[AAOpusAudioPlayer#%p op_open_file failed: %d]", self, openError);
//                [self cleanupAndReportError];
//
//                return;
//            }
...
//            }
//            _filledAudioBufferCount = AAOpusAudioPlayerBufferCount;
//            _filledAudioBufferPosition = 0;
//
//            _finished = false;
//
//            if (_isSeekable && position >= 0.0)
//                op_pcm_seek(_opusFile, (ogg_int64_t)(position * AAOpusAudioPlayerSampleRate));
//
...
//            if (_isSeekable && position >= 0.0)
//            {
//                int result = op_pcm_seek(_opusFile, (ogg_int64_t)(position * AAOpusAudioPlayerSampleRate));
//                if (result != OPUS_OK)
...
//    Float64 seconds = 0.4;
//
//    Float64 numPacketsForTime = AAOpusAudioPlayerSampleRate * seconds;
...
//    {
//        OSSpinLockLock(&audioPositionLock);
//        result = _currentPcmOffset / (NSTimeInterval)AAOpusAudioPlayerSampleRate;
//        OSSpinLockUnlock(&audioPositionLock);
//    };
//
//    if (sync)
...
//{
//    return _totalPcmDuration / (NSTimeInterval)AAOpusAudioPlayerSampleRate;
//}
//
//@end
...
static std::map<intptr_t, __weak AAOpusAudioPlayerAU *> activeAudioPlayers;
    ...
    [[AAAudioPlayer _playerQueue] dispatchOnQueue:^
     {
         if (audioUnit != NULL)
         {
             ...
             status = AudioOutputUnitStop(audioUnit);
             if (status != noErr)
                 NSLog(@"[AAOpusAudioPlayer#%x AudioOutputUnitStop failed: %d]", objectId, (int)status);
    ...
    [[AAAudioPlayer _playerQueue] dispatchOnQueue:^
     {
         ...
         if (_audioUnit == NULL)
         {
             ...
             if (status != noErr)
             {
                 NSLog(@"[AAOpusAudioPlayer#%x AudioUnitSetProperty kAudioUnitProperty_StreamFormat failed: %d]", self, (int)status);
                 [self cleanupAndReportError];
             ...
             if (AudioUnitSetProperty(_audioUnit, kAudioUnitProperty_SetRenderCallback, kAudioUnitScope_Global, kOutputBus, &callbackStruct, sizeof(callbackStruct)) != noErr)
             {
                 NSLog(@"[AAOpusAudioPlayer#%x AudioUnitSetProperty kAudioUnitProperty_SetRenderCallback failed]", self);
                 [self cleanupAndReportError];
