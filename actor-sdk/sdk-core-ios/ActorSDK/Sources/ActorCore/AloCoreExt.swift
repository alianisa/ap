...
//        let resized = imageFromData!.resizeOptimize(1200 * 1200);
        let resizedNone = imageFromData!
        let resizedLarge = imageFromData!.resizeSquare(1920, maxH: 1024);
        let resizedMedium = imageFromData!.resizeSquare(1920, maxH: 1024);
        let resizedSmall = imageFromData!.resizeOptimize(1920 * 1920);
        
        let quality = Actor.getQuality()
        ...
        if (quality == "none") {
            _ = animated ? ((try? image.write(to: URL(fileURLWithPath: path), options: [.atomic])) != nil) : ((try? resizedNone.jpegData(compressionQuality: 1)!.write(to: URL(fileURLWithPath: path), options: [.atomic])) != nil)
            
            animated ? sendAnimation(with: peer, withName: "image.gif", withW: jint(resizedNone.size.width), withH:jint(resizedNone.size.height), with: fastThumb, withDescriptor: descriptor) : sendPhoto(with: peer, withName: "image.jpg", withW: jint(resizedNone.size.width), withH: jint(resizedNone.size.height), with: fastThumb, withDescriptor: descriptor)
        } else if (quality == "large") {
            _ = animated ? ((try? image.write(to: URL(fileURLWithPath: path), options: [.atomic])) != nil) : ((try? resizedLarge.jpegData(compressionQuality: 0.9)!.write(to: URL(fileURLWithPath: path), options: [.atomic])) != nil)
            
            animated ? sendAnimation(with: peer, withName: "image.gif", withW: jint(resizedLarge.size.width), withH:jint(resizedLarge.size.height), with: fastThumb, withDescriptor: descriptor) : sendPhoto(with: peer, withName: "image.jpg", withW: jint(resizedLarge.size.width), withH: jint(resizedLarge.size.height), with: fastThumb, withDescriptor: descriptor)
        } else if (quality == "medium") {
            _ = animated ? ((try? image.write(to: URL(fileURLWithPath: path), options: [.atomic])) != nil) : ((try? resizedMedium.jpegData(compressionQuality: 0.7)!.write(to: URL(fileURLWithPath: path), options: [.atomic])) != nil)
            
            animated ? sendAnimation(with: peer, withName: "image.gif", withW: jint(resizedMedium.size.width), withH:jint(resizedMedium.size.height), with: fastThumb, withDescriptor: descriptor) : sendPhoto(with: peer, withName: "image.jpg", withW: jint(resizedMedium.size.width), withH: jint(resizedMedium.size.height), with: fastThumb, withDescriptor: descriptor)
        } else if (quality == "small") {
            _ = animated ? ((try? image.write(to: URL(fileURLWithPath: path), options: [.atomic])) != nil) : ((try? resizedSmall.jpegData(compressionQuality: 0.7)!.write(to: URL(fileURLWithPath: path), options: [.atomic])) != nil)
            
            animated ? sendAnimation(with: peer, withName: "image.gif", withW: jint(resizedSmall.size.width), withH:jint(resizedSmall.size.height), with: fastThumb, withDescriptor: descriptor) : sendPhoto(with: peer, withName: "image.jpg", withW: jint(resizedSmall.size.width), withH: jint(resizedSmall.size.height), with: fastThumb, withDescriptor: descriptor)
        }
    ...
    public func sendVideo(_ url: URL, peer: ACPeer) {
        ...
        if let videoData = try? Data(contentsOf: url) { // if data have on this local path url go to upload
            ...
            do {
                ...
                let thumbData = thumb.jpegData(compressionQuality: 0.55); // thumbnail binary data
    ...
    fileprivate func prepareAvatar(_ image: UIImage) -> String {
        ...
        try? thumb.jpegData(compressionQuality: 0.8)!.write(to: URL(fileURLWithPath: avatarPath), options: [.atomic])
        return res
    }
